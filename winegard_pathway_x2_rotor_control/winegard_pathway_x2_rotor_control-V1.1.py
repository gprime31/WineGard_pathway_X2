...
class CarryoutRotor:
    ...
    def get_position(self):
        self.initialize()
        self.send_command('')  # ENTER
        response = self.send_command('a')  # unified az + el request

        results = re.findall(r'[-+]?\d*\.\d+|\d+', response)
        if len(results) >= 2:
            try:
                self.last_az = float(results[0])
                self.last_el = float(results[1])
            except ValueError:
                print("Error parsing angles")
        else:
            print("Could not parse az/el from combined 'a' command")
